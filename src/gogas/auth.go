...
func getClient(ctx context.Context, config *oauth2.Config) *http.Client {
	tok := getOauth2Token(config)
	return config.Client(ctx, tok)
}

// getOauth2Token
// gets an Oauth2 token either from a cache file or the web
func getOauth2Token(config *oauth2.Config) *oauth2.Token {
...
func tokenCacheFile() (string, error) {
	...
	return filepath.Join(tokenCacheDir,
		url.QueryEscape("gogas-auth.json")), err
}
